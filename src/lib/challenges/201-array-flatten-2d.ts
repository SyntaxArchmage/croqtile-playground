...
export const challenge201: Challenge = {
  id: "c201",
  title: "Array Flatten 2D",
  difficulty: "easy",
  description: `Flatten a 2×3 matrix into a 1D array in **row-major** order using \`parallel\`.
...
Use \`parallel {i, j} by [2, 3]\` and map each element to \`flat[i * 3 + j]\`.`,
  starterCode: `__co__ void array_flatten_2d() {
...
`,
  tests: [
    ...
    { expectedOutput: "flat[3] = 4", description: "Start of second row" },
    {
      ...
      description: "Full row-major flattened output",
    },
  ],
